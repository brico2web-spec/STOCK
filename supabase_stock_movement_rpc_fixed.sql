-- شغّل هذا السكريبت كاملاً مرة واحدة في Supabase SQL Editor.
-- يصلح دالة دخول وخروج السلع ويضمن صلاحية manager/admin.
...
begin
  ...
  v_old := coalesce(v_product.total_pieces, 0);
  v_boxes := greatest(coalesce(p_boxes_quantity, 0), 0);
  v_delta := case
  ...
  set total_pieces = v_new,
      boxes_quantity = greatest(0, floor(v_new::numeric / greatest(pieces_per_box, 1))::integer),
      updated_at = now()
  where id = p_product_id;
  ...
  insert into public.stock_movements (
    ...
    p_product_id, p_movement_type, v_delta, v_boxes,
  ...
  insert into public.stock_change_log (
    ...
    jsonb_build_object('total_pieces', v_new,
                       'boxes_quantity', floor(v_new::numeric / greatest(v_product.pieces_per_box, 1))::integer),
    auth.uid()
  );
